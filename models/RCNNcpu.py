...
class RCNN(nn.Module):
	def __init__(self, batch_size, output_size, hidden_size, vocab_size, embedding_length, weights):
		...
		self.dropout = 0.8
		self.lstm = nn.LSTM(embedding_length, hidden_size, num_layers=3, dropout=self.dropout, bidirectional=True)
	...
	def forward(self, input_sentence, batch_size=None):
		...
		if batch_size is None:
			h_0 = Variable(torch.zeros(2, self.batch_size, self.hidden_size).cpu()) # Initial hidden state of the LSTM
			c_0 = Variable(torch.zeros(2, self.batch_size, self.hidden_size).cpu()) # Initial cell state of the LSTM
		else:
			h_0 = Variable(torch.zeros(2, batch_size, self.hidden_size).cpu())
			c_0 = Variable(torch.zeros(2, batch_size, self.hidden_size).cpu())
